...
var sum = function(array) {
	...
	copyArray[0] += firstElement;
	return sum(copyArray);
};
...
var range = function(x, y) {
	if (Array.isArray(x)) {
		var lastNumLessThanY = x[x.length-1] < y;
		var nextNum = lastNumLessThanY ? x[x.length-1]+1 : x[x.length-1]-1;
		if (nextNum === y) {
			return x;
		}
		x.push(nextNum);
	...
	return x < y ? range([x+1],y) : range([x-1],y);
};
...
var palindrome = function(string) {
	string = string.slice().toLowerCase();
